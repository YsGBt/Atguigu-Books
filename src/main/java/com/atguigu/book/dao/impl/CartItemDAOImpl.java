...
public class CartItemDAOImpl extends BaseDAO<CartItem> implements CartItemDAO {
  ...
  public boolean addCartItem(Connection conn, CartItem cartItem) {
    ...
    int count = update(conn, sql, cartItem.getBookId(), cartItem.getBuyCount(), cartItem.getUserBeanId());
    return count == 1;
  }
  ...
  public boolean updateCartItem(Connection conn, CartItem cartItem) {
    ...
    int count = update(conn, sql, cartItem.getBookId(), cartItem.getBuyCount(), cartItem.getUserBeanId(), cartItem.getId());
    return count == 1;
  }
